...
def extract_current_stocks_data():
    ...
    # Change date format
    df_stocks_name['last_quote_date'] = pd.to_datetime(df_stocks_name['last_quote_date'])
    df_stocks_name['last_quote_date'] = df_stocks_name['last_quote_date'].dt.strftime('%d-%b-%y')
    
    # Reorder columns
    df_stocks_name = df_stocks_name[["symbol", "name", "last_quote_date", "is_prices_uptodate"]]
